...
describe("getNetsForMonth", () => {
	...
	it("one-time event appears only on its date", () => {
		const events = parseIcs(
			buildIcs([makeNet({ schedule: "2026-03-15 12:00 30m" })]),
		);
		const dayMap = getNetsForMonth(events, 2026, 2);
	...
	it("one-time event in another month does not appear", () => {
		const events = parseIcs(
			buildIcs([makeNet({ schedule: "2026-06-12 12:00 30m" })]),
		);
	...
	it("events within a day are sorted by start time", () => {
		const nets = [
			makeNet({
				fileSlug: "late",
				title: "Late Net",
				schedule: "Thu 21:00 30m",
			}),
			makeNet({
				fileSlug: "early",
				title: "Early Net",
				schedule: "Thu 18:00 30m",
			}),
		];
